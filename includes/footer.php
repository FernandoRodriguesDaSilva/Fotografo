<!-- RODAPÉ PRINCIPAL DA PÁGINA -->
<?php 
function data(){
  date_default_timezone_set('America/Sao_Paulo');
  echo date('d/m/Y H:i:s').'<br/>';
}
?>

<footer class="page-footer blue">
  <div class="container">
    <div class="row">
      <div class="col l6 s12">
        <h5 class="white-text">Richard Thompsom</h5>
        <p class="grey-text text-lighten-4">Fotografar é assim<br>poesia composta de luz.</p>
      </div>
      <div class="col l4 offset-l2 s12">
        <h5 class="white-text">Links</h5>
        <ul>
          <li><a class="grey-text text-lighten-3" href="index.php">Home</a></li>
          <li><a class="grey-text text-lighten-3" href="contato.php">Contato</a></li>
          <li><a class="grey-text text-lighten-3" href="casamento.php">Casamentos</a></li>
          <li><a class="grey-text text-lighten-3" href="evento.php">Eventos</a></li>
          <li><a class="grey-text text-lighten-3" href="galeria.php">Galeria</a></li>
          <li><a class="grey-text text-lighten-3" href="books.php">books</a></li>
        </ul>
      </div>
    </div>
  </div>
  <div class="footer-copyright">
    <div class="container">
      Todos os direitos reservados  © 2018  - Fotografo
      <a class="grey-text text-lighten-4 right" href="#!">
        <?php data(); ?>
      </a>
    </div>
  </div>
</footer>
<!-- IMPORTANDO JQUERY E JAVASCRIPT -->
<script src="js/jquery-3.3.1.min.js"></script>
<script src="js/materialize.min.js"></script>

<!-- CONFIGURAÇÕES JQUERY E JAVASCRIPT -->
<script>
  $('document').ready(function(){
   $('.modal').modal();
   $('.button-collapse').sideNav();
   $('.slider').slider();
   $('.materialboxed').materialboxed();
 });
</script>
</body>
</html>